...
func (r *candidateRepository) CreateCandidate(candidate *models.CandidateSignUpRequest) error {
	...
	query = `INSERT INTO candidates (public_id, current_position, resume, bio) VALUES ($1, $2, $3, $4) RETURNING id;`
	err = tx.QueryRow(ctx, query, user_public_id, candidate.CurrentPosition, candidate.Resume, candidate.Bio).Scan(&candidate_id)
	if err != nil {
